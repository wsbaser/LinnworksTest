...
namespace automateit.SCSS
{
    public class ScssBuilder
    {
        ...
        public static Scss Create(string scssSelector)
        {
            ...
            {
                // NOT A CSS SELECTOR
                return CreateFromXpath(scssSelector, combineWithRoot);
            }

            foreach (var partScssSelector in parts)
            {
                var xpathPart = ScssPartToXpath(partScssSelector, out var partScssIsTrueCss);
                if (xpathPart == null)
                {
                    // NOT A CSS SELECTOR
                    return CreateFromXpath(scssSelector, combineWithRoot);
            ...
            return new Scss(xpath, isTrueCss ? scssSelector : null, combineWithRoot);
        }
...
//                    break;
            ...
            if (!Validate(tag, id, classNames, attributes, function, functionArgument))
            {
                return null;
            }
        ...
        private static bool Validate(string tag, string id, List<string> classNames, List<ScssAttribute> attributes,
            string function, string functionArgument)
        {
            if (!string.IsNullOrEmpty(tag))
            {
                if (!IsElementName(tag))
                {
                    return false;
                }
            }
            ...
            {
                if (!IsCSSFunction(function, functionArgument))
                {
                    return false;
                }
            }
        ...
        private static bool IsCSSFunction(string value, string functionArgument) {
            switch (value) {
                case CONTAINS:
                    return IsContainsArgument(functionArgument);
                case NTH_CHILD:
                    return IsNthChildArgument(functionArgument);
                default:
                    return false;
//                    throw new InvalidScssException("'{0}' is not css function", value);
        ...
        private static string XpathTextCondition(string text)
        {
            ...
            var textWithNoQuotes = text.CutFirst().CutLast();
            // TODO: need tests for this change
            if (textWithNoQuotes.Contains("'"))
            {
